...
bool Is_Likely_SBUS_Footer(uint8_t footer)//帧尾粗校验函数
{
    return (footer == 0x00U) || ((footer & 0x0FU) == 0x04U);
}

//16通道不是一一对应2字节 是11bit连续打包
uint16_t Extract_SBUS_Channel(Struct_SBUS_UART_Rx_Data *raw_frame, uint8_t channel_index)//位提取函数 最关键
{
...
void Class_SBUS::Judge_Update(const Struct_SBUS_Rx_Data &Pre_Data, const Struct_SBUS_Rx_Data &Now_Data)
{
    if (Pre_Data.Channel[SBUS_CH_RIGHT_X - 1U] == Now_Data.Channel[SBUS_CH_RIGHT_X - 1U] &&
        ...
        Pre_Data.Channel[SBUS_CH_AUX_KNOB_1 - 1U] == Now_Data.Channel[SBUS_CH_AUX_KNOB_1 - 1U] &&
        Pre_Data.Channel[SBUS_CH_AUX_KNOB_2 - 1U] == Now_Data.Channel[SBUS_CH_AUX_KNOB_2 - 1U] &&
...
void Class_SBUS::SBUS_Data_Process()
{
    ...
    Rx_Data.VRA = Normalize_Channel(Get_Channel_Value(SBUS_CH_AUX_KNOB_1), SBUS_SIGN_VRA);
    Rx_Data.VRB = Normalize_Channel(Get_Channel_Value(SBUS_CH_AUX_KNOB_2), SBUS_SIGN_VRB);
    // Keep Yaw aligned with the active chassis yaw source used by upper-layer control.
    Rx_Data.Yaw = Rx_Data.Right_X;
...
void Class_SBUS::TIM1msMod100_Alive_PeriodElapsedCallback()
{
